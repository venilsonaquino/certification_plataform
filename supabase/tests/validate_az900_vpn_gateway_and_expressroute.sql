...
where certification.code = 'az-900' and domain.title = 'Describe Azure architecture and services'
  and topic.title = 'Networking Services' and lesson.slug = 'vpn-gateway-vs-expressroute';

do $$
declare visual_config jsonb; duplicate_count integer;
begin
  if not exists (select 1 from supabase_migrations.schema_migrations where version = '20260827070000') then
    raise exception '8.7.4 migration is not registered';
  end if;
  if (select count(*) from target_lesson) <> 1
    or exists (select 1 from target_lesson target join public.lessons lesson on lesson.id = target.id where not lesson.is_published or lesson.content is null or btrim(lesson.content) = '' or lesson.estimated_minutes <> 12) then
  ...
  if (select array_agg(block.type order by block.display_order) from public.lesson_content_blocks block join target_lesson target on target.id = block.lesson_id where block.is_published)
    is distinct from array['explanation','important','example','explanation','example','important','visual_experience','exam_trap','exam_tip','summary']::text[] then
    raise exception 'Unexpected Content Block sequence';
  end if;
  if exists (
    select 1 from target_lesson target join public.lesson_content_blocks block on block.lesson_id = target.id
    group by target.id having count(*) <> 10 or min(block.display_order) <> 1 or max(block.display_order) <> 10
      ...
      or count(*) filter (where block.type = 'visual_experience' and block.visual_experience_id = '76000000-0000-4000-8000-000000000011') <> 1
  ) then raise exception 'Block ordering or required blocks are invalid'; end if;
  ...
  where visual.id = '76000000-0000-4000-8000-000000000011' and visual.type = 'comparison' and visual.is_published;
  if jsonb_typeof(visual_config -> 'columns') <> 'array' or jsonb_typeof(visual_config -> 'rows') <> 'array'
    or jsonb_array_length(visual_config -> 'columns') <> 2 or jsonb_array_length(visual_config -> 'rows') <> 7 then
    raise exception 'VPN/ExpressRoute visual shape is invalid';
  end if;
  if exists (
    select 1 from jsonb_array_elements(visual_config -> 'rows') row_config
    where not (row_config -> 'values' ? 'vpn-gateway') or not (row_config -> 'values' ? 'expressroute')
      or jsonb_object_length(row_config -> 'values') <> 2
  ) then raise exception 'A visual comparison row does not cover both columns'; end if;

  if (select count(*) from public.flashcards card join target_lesson target on target.id = card.lesson_id where card.is_published) <> 4
    or not exists (select 1 from public.flashcards where id = '72000000-0000-4000-8000-000000000030' and back_text ilike '%S2S%rede%P2S%dispositivo%')
    or not exists (select 1 from public.flashcards where id = '72000000-0000-4000-8000-000000000031' and back_text ilike '%privada%provider%') then
    raise exception 'The four preserved Flashcards are incomplete';
  end if;
  if exists (
    select 1 from public.questions question join target_lesson target on target.id = question.lesson_id
  ...
  ) then raise exception 'A Question or its options are invalid'; end if;
  ...
  if exists (
    ...
    where concat_ws(' ',question.question_text,question.explanation,option.option_text,option.explanation)
      ~* '(BGP|ExpressRoute Direct|FastPath|SKU|throughput num.rico|circuit configuration|IPsec/IKE|OpenVPN|certificado)'
  ) then raise exception 'Out-of-scope networking practice remains'; end if;
  select count(*) into duplicate_count from (
    select lower(regexp_replace(btrim(question.question_text), '[^[:alnum:]]+', ' ', 'g')) from public.questions question
    where question.topic_id = '32000000-0000-4000-8000-000000000003' group by 1 having count(*) > 1
  ) duplicates;
...
values ('00000000-0000-0000-0000-000000000000','58000000-0000-4000-8000-000000000015','authenticated','authenticated','vpn-expressroute-quiz@example.invalid','',now(),'{"provider":"email","providers":["email"]}'::jsonb,'{}'::jsonb,now(),now());
set local role authenticated;
select set_config('request.jwt.claim.sub','58000000-0000-4000-8000-000000000015',true);

do $$
declare target_id uuid; lesson_attempt public.quiz_attempts; topic_attempt public.quiz_attempts;
begin
  ...
  if (select count(*) from public.lesson_content_blocks where lesson_id = target_id and is_published) <> 10
    ...
    or (select count(*) from public.flashcards where lesson_id = target_id and is_published) <> 4 then
    raise exception 'Authenticated users cannot read all published 8.7.4 content';
  end if;
  ...
  if lesson_attempt.user_id <> auth.uid() or lesson_attempt.lesson_id <> target_id or lesson_attempt.total_questions <> 5
    or lesson_attempt.status <> 'in_progress' or (select count(*) from public.quiz_attempt_questions where attempt_id = lesson_attempt.id) <> 5 then
    raise exception 'VPN/ExpressRoute Lesson Quiz failed';
  end if;
  ...
  if topic_attempt.user_id <> auth.uid() or topic_attempt.topic_id <> '32000000-0000-4000-8000-000000000003'
    or topic_attempt.total_questions <> 10 or (select count(*) from public.quiz_attempt_questions where attempt_id = topic_attempt.id) <> 10
    or (select count(distinct question.lesson_id) from public.quiz_attempt_questions item join public.questions question on question.id = item.question_id where item.attempt_id = topic_attempt.id) <> 4 then
    raise exception 'Networking Topic Quiz failed after 8.7.4';
  end if;
end; $$;
reset role;

select json_build_object('stage','8.7.4','lesson','vpn-gateway-vs-expressroute','published_blocks',10,
  'visuals_created',1,'flashcards_preserved_and_corrected',4,'questions_added',5,'history_preserved',true) as vpn_expressroute_validation;
rollback;
