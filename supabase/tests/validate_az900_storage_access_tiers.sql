...
where certification.code='az-900' and domain.title='Describe Azure architecture and services'
  and topic.id='32000000-0000-4000-8000-000000000004' and lesson.slug='storage-tiers';

do $$ declare duplicate_count integer;
begin
  if not exists(select 1 from supabase_migrations.schema_migrations where version='20260828020000') then raise exception '8.8.3 migration is not registered'; end if;
  if (select count(*) from target_lesson)<>1 or exists(select 1 from target_lesson target join public.lessons lesson on lesson.id=target.id
    ...
    raise exception 'Storage Tiers Lesson publication, fallback or estimate is invalid'; end if;
  if (select array_agg(block.type order by block.display_order) from public.lesson_content_blocks block join target_lesson target on target.id=block.lesson_id where block.is_published)
    is distinct from array['explanation','explanation','explanation','explanation','explanation','important','example','important','exam_trap','exam_tip','summary']::text[] then
    raise exception 'Unexpected Storage Tiers block sequence'; end if;
  if exists(select 1 from target_lesson target join public.lesson_content_blocks block on block.lesson_id=target.id group by target.id
    having count(*)<>11 or min(block.display_order)<>1 or max(block.display_order)<>11 or count(distinct block.display_order)<>11
      or count(*) filter(where block.is_published)<>11 or count(*) filter(where block.type='exam_tip')<>1
      or count(*) filter(where block.type='exam_trap')<>1 or count(*) filter(where block.type='summary'
        and jsonb_typeof(block.config->'items')='array' and jsonb_array_length(block.config->'items') between 3 and 6)<>1) then
    raise exception 'Storage Tiers blocks, order or summary are invalid'; end if;
  if exists(select 1 from public.visual_experiences visual join target_lesson target on target.id=visual.lesson_id)
    or exists(select 1 from public.lesson_content_blocks block join target_lesson target on target.id=block.lesson_id where block.type='visual_experience') then
    raise exception '8.8.3 must not contain a Visual Experience'; end if;
  if (select count(*) from public.flashcards card join target_lesson target on target.id=card.lesson_id where card.is_published)<>6
    or not exists(select 1 from public.flashcards where id='7e300000-0000-4000-8000-000000000003' and back_text ilike '%Cold%online%Archive%offline%')
    or not exists(select 1 from public.flashcards where id='7e300000-0000-4000-8000-000000000006' and back_text ilike '%não%access tier%') then
    raise exception 'Storage Tiers Flashcards are incomplete'; end if;
  select count(*) into duplicate_count from(select lower(regexp_replace(btrim(card.front_text),'[^[:alnum:]]+',' ','g'))
    from public.flashcards card join target_lesson target on target.id=card.lesson_id group by 1 having count(*)>1) duplicates;
  if duplicate_count<>0 then raise exception 'Storage Tiers contains exact Flashcard duplicates'; end if;
  if (select count(*) from public.questions question join target_lesson target on target.id=question.lesson_id where question.is_published)<>5
    ...
    raise exception 'Storage Tiers Question distribution is invalid'; end if;
  if exists(select 1 from public.questions question join target_lesson target on target.id=question.lesson_id
    ...
    having count(option.id)<>4 or count(option.id) filter(where option.is_correct)<>1 or count(distinct lower(btrim(option.option_text)))<>4
      or min(length(btrim(question.explanation)))<40) then raise exception 'A Storage Tiers Question or options are invalid'; end if;
  if exists(with artifacts as(
    ...
    select 1 from artifacts where text ~* '(cold (é|=) (um )?tier offline|archive (é|=) (um )?backup|archive.{0,40}(leitura|acesso) imediat|hot sempre.{0,20}mais rápid|smart tier.{0,30}(obrigatório|principal))') then
    raise exception 'A prohibited access-tier misconception remains'; end if;
  if exists(select 1 from public.questions question join target_lesson target on target.id=question.lesson_id
    ...
    where concat_ws(' ',question.question_text,question.explanation,option.option_text,option.explanation)
      ~* '(lifecycle management|regra de lifecycle|preço por GB|IOPS|throughput|LRS|ZRS|GRS|GZRS|AzCopy|Storage Explorer|File Sync|Data Box|Azure Migrate)') then
    raise exception 'Storage Tiers practice exceeds the 8.8.3 scope'; end if;
end; $$;
...
do $$ begin
  ...
  if has_table_privilege('authenticated','public.questions','SELECT') or has_table_privilege('authenticated','public.question_options','SELECT')
    ...
    raise exception 'Storage Tiers curriculum grants are invalid'; end if;
end; $$;

insert into auth.users(instance_id,id,aud,role,email,encrypted_password,email_confirmed_at,raw_app_meta_data,raw_user_meta_data,created_at,updated_at)
values('00000000-0000-0000-0000-000000000000','58000000-0000-4000-8000-000000000021','authenticated','authenticated','storage-tiers-883@example.invalid','',now(),'{"provider":"email","providers":["email"]}'::jsonb,'{}'::jsonb,now(),now());
...
begin
  ...
  if lesson_attempt.total_questions<>5 or (select count(*) from public.quiz_attempt_questions where attempt_id=lesson_attempt.id)<>5 then raise exception 'Storage Tiers Lesson Quiz failed'; end if;
  ...
  if topic_attempt.total_questions<>10 or (select count(*) from public.quiz_attempt_questions where attempt_id=topic_attempt.id)<>10
    or (select count(distinct question.lesson_id) from public.quiz_attempt_questions item join public.questions question on question.id=item.question_id where item.attempt_id=topic_attempt.id)<>6 then
    raise exception 'Storage Topic Quiz failed after 8.8.3'; end if;
end; $$;
reset role;
select json_build_object('stage','8.8.3','lesson','storage-tiers','published_blocks',11,'visuals_created',0,
  'flashcards_added',6,'questions_added',5,'difficulty',json_build_object('easy',2,'medium',2,'hard',1),'history_preserved',true) as storage_tiers_validation;
rollback;
