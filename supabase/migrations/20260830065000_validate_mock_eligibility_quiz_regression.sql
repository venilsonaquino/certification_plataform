...
begin
  if pg_get_functiondef('public.start_lesson_quiz(uuid)'::regprocedure) like '%mock_eligible%'
    ...
    raise exception '11.3 changed an existing study selector';
  end if;
end;
$$;
...
values('00000000-0000-0000-0000-000000000000','65000000-0000-4000-8000-000000000001',
  'authenticated','authenticated','mock-eligibility-regression@example.invalid','',now(),
  '{"provider":"email","providers":["email"]}'::jsonb,'{}',now(),now());

create temporary table audit_113_answer_key on commit drop as
select question.id question_id,
  (array_agg(option.id order by option.display_order) filter(where option.is_correct))[1] correct_id,
...
grant select on audit_113_answer_key to authenticated;
...
begin
  ...
  if v_lesson.total_questions<>5 then raise exception '11.3 Lesson Quiz regressed'; end if;
  ...
  if v_topic_one.total_questions<>10 then raise exception '11.3 Topic Quiz regressed'; end if;

  for v_item in
    select item.question_id,key.incorrect_id
    from public.quiz_attempt_questions item
    join audit_113_answer_key key on key.question_id=item.question_id
  ...
  if v_topic_two.id=v_topic_one.id or v_topic_two.total_questions<>10
    ...
    raise exception '11.3 Topic retake rotation regressed';
  end if;

  select * into strict v_review from public.start_review_quiz(v_certification_id,null);
  if v_review.total_questions not between 5 and 10 then raise exception '11.3 Review regressed'; end if;
  ...
  if not exists(select 1 from public.user_flashcard_progress
      where user_id='65000000-0000-4000-8000-000000000001' and flashcard_id=v_card) then
    raise exception '11.3 Spaced Repetition regressed';
  end if;
end;
$$;
