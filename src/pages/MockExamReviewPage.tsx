...
const filters: readonly { value: ReviewFilter; label: string }[] = [
  { value: 'needs_review', label: 'Incorrect + Unanswered' },
  { value: 'all', label: 'All' },
...
export function MockExamReviewPage() {
  ...
  if (loading) return <CertificationDataState title="Loading Mock review..." loading />
  if (error) return <CertificationDataState title="Review indisponível." description={error} onRetry={() => void loadReview()} />
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <h2 ref={resultHeadingRef} tabIndex={-1} className="mt-5 text-sm font-semibold text-slate-600 outline-none" aria-live="polite">
        {visibleQuestions.length} de {questions.length} Questions exibidas
      </h2>
      <div className="mt-4 grid gap-5">
        ...
        {visibleQuestions.length === 0 && <p className="rounded-2xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-600">Nenhuma Question corresponde a este filtro.</p>}
      </div>
    </div>
  )
}
