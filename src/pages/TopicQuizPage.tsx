...
export function TopicQuizPage() {
  ...
  const quiz = useTopicQuiz(topic?.id ?? null)
  const backRoute = certificationRoute(currentCertification.code, 'study')

  if (contentLoading || quiz.loading) return <CertificationDataState title="Preparando o Quiz do Tópico..." loading />
  ...
  if (quiz.unavailable) return <div className="mx-auto max-w-3xl"><CertificationDataState title="Ainda não existem questões suficientes para este tópico." /><Link to={backRoute} className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-blue-700"><ArrowLeft className="h-4 w-4" />Voltar para a trilha</Link></div>
  if (!quiz.data || quiz.error && !quiz.currentQuestion) return <CertificationDataState title="Não foi possível carregar o Quiz." description={quiz.error ?? undefined} onRetry={quiz.retry} />
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <header className="mt-5 border-b border-slate-200 pb-6">
        <p className="flex items-center gap-2 text-sm font-bold text-violet-700"><BrainCircuit className="h-5 w-5" />{formatCertificationCode(currentCertification.code)} · Quiz do Tópico</p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl">{topic.title}</h1>
        <p className="mt-2 text-sm text-slate-500">{data.attempt.totalQuestions} questões combinando as aulas deste tópico.</p>
      </header>
      {quiz.showResult ? (
        <div className="mt-7"><TopicQuizResult data={data} certificationCode={currentCertification.code} restarting={quiz.loading} onRestart={() => { void quiz.restart() }} /></div>
      ) : currentQuestion ? (
        <div className="mt-7">
