...
describe('MockExamReviewPage', () => {
  ...
  it('começa em incorretas + não respondidas e filtra todos os estados', async () => {
    ...
    expect(await screen.findByText('2 de 3 questões exibidas')).toBeInTheDocument()
    ...
    expect(screen.getByText('3 de 3 questões exibidas')).toBeInTheDocument()
    expect(screen.getByText('Question 3?')).toBeInTheDocument()
    await userEvent.click(screen.getByRole('button', { name: 'Incorretas' }))
    expect(screen.getByText('1 de 3 questões exibidas')).toBeInTheDocument()
    expect(screen.getByText('Question 1?')).toBeInTheDocument()
    await userEvent.click(screen.getByRole('button', { name: 'Não respondidas' }))
    expect(screen.getByText('Question 2?')).toBeInTheDocument()
    await userEvent.click(screen.getByRole('button', { name: 'Corretas' }))
    expect(screen.getByText('Question 3?')).toBeInTheDocument()
  })
})
