  ...
  return (
    <section aria-labelledby="recent-performance-title" className="mt-10 grid gap-5 lg:grid-cols-[1.25fr_0.75fr]">
      <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card sm:p-6">
        <h2 id="recent-performance-title" className="text-xl font-bold text-slate-950">Recent Mock performance</h2>
            ...
            {attempts.map((attempt) => (
              <li key={attempt.attemptId} className="flex items-center justify-between gap-4 rounded-xl bg-slate-50 px-4 py-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 flex items-center gap-1 text-xs text-slate-500"><CalendarDays aria-hidden="true" className="h-3.5 w-3.5" />{formatReadinessDate(attempt.evaluatedAt)} · {attempt.status === 'expired' ? 'Expired' : 'Completed'}</p>
                </div>
      ...
      <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card sm:p-6">
        ...
        <h2 className="mt-4 text-lg font-bold text-slate-950">Trend: {trendLabels[trend]}</h2>
