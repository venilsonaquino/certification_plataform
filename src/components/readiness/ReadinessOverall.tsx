    ...
    <>
      <section aria-labelledby="overall-readiness-title" className="mt-8 overflow-hidden rounded-3xl border border-blue-200 bg-gradient-to-br from-navy-900 via-blue-900 to-blue-700 p-6 text-white shadow-xl shadow-blue-900/10 sm:p-8 lg:mt-10">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-blue-200">Overall Readiness</p>
        ...
        <div className="mt-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <OverallFact icon={ClipboardCheck} label="Evidence" value={evidenceLabels[readiness.evidenceLevel]} />
          <OverallFact icon={Activity} label="Trend" value={trendLabels[readiness.trend]} />
          <OverallFact icon={Layers3} label="Assessment activity" value={`${mockAttempts} Mocks · ${topicAttempts} Topic Quizzes`} />
          <OverallFact icon={CalendarClock} label="Last evaluated" value={formatReadinessDate(readiness.trace.latestEvidenceAt)} />
        </div>
      ...
      <section aria-labelledby="evidence-title" className="mt-5 grid gap-5 lg:grid-cols-[1fr_1.15fr]">
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card sm:p-6">
          <h2 id="evidence-title" className="text-lg font-bold text-slate-950">Evidence & learning progress</h2>
          ...
          <div className="mt-5 flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">Learning Progress</p>
            </div>
            <p className="text-right text-sm font-semibold text-slate-600">
              {progress.completedLessons} de {progress.totalLessons} aulas
            </p>
          </div>
          <div role="progressbar" aria-label="Learning Progress" aria-valuemin={0} aria-valuemax={100} aria-valuenow={learningPercentage} className="mt-4 h-2 overflow-hidden rounded-full bg-slate-100">
            <div className="h-full rounded-full bg-gradient-to-r from-blue-600 to-sky-400" style={{ width: `${learningPercentage}%` }} />
          </div>
          <p className="mt-4 text-xs text-slate-500">
            {readiness.trace.answeredQuestions} respostas avaliadas · {readiness.trace.distinctQuestions} Questions distintas · {readiness.trace.assessmentSessions} sessões
          </p>
        </article>

        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card sm:p-6">
          <h2 className="text-lg font-bold text-slate-950">Recommended next actions</h2>
